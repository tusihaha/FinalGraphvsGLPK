...
def get_info(folder, number, count, first_char):
    time_used_glpk = []
    timeslot_used_glpk = []
    for i in range(1, count + 1):
        f = open('./' + folder + '/' + first_char + str(number + 1) + 'glpk_result' + str(int(i)) + '.txt', 'r')
        lines = f.readlines()
        for line in lines:
            c = line.split(' ')
            while '' in c:
    ...
    timeslot_used_graph = []
    for i in range(1, count + 1):
        f = open('./' + folder + '/' + first_char + str(number + 1) + 'graph_result' + str(int(i)) + '.txt', 'r')
        lines = f.readlines()
        for line in lines:
            c = line.split(' ')
            while '' in c:
...
if __name__ == "__main__":
    ...
    for i in range(len(series)):
        fig, axs = plt.subplots(4,1, gridspec_kw={'height_ratios': [5, 1, 1, 1]})

        [time_used_glpk1, timeslot_used_glpk, time_used_graph1, timeslot_used_graph] = get_info('Result6', i, len(count), first_char)

        axs[1].axis('tight')
        axs[1].axis('off')
        columns = tuple(count)
        rows = ['GLPK', 'Graph']
        table = axs[1].table(cellText = [timeslot_used_glpk, timeslot_used_graph], cellLoc = 'center', rowLabels = rows, colLabels = columns, loc='bottom')
        table.auto_set_font_size(False)
        table.set_fontsize(8)
        table.scale(1, 1.3)

        [time_used_glpk2, timeslot_used_glpk, time_used_graph2, timeslot_used_graph] = get_info('Result7', i, len(count), first_char)

        axs[2].axis('tight')
        axs[2].axis('off')
        columns = tuple(count)
        rows = ['GLPK', 'Graph']
        table = axs[2].table(cellText = [timeslot_used_glpk, timeslot_used_graph], cellLoc = 'center', rowLabels = rows, colLabels = columns, loc='center')
        table.auto_set_font_size(False)
        table.set_fontsize(8)
        table.scale(1, 1.3)

        [time_used_glpk3, timeslot_used_glpk, time_used_graph3, timeslot_used_graph] = get_info('Result8', i, len(count), first_char)

        axs[3].axis('tight')
        axs[3].axis('off')
        columns = tuple(count)
        rows = ['GLPK', 'Graph']
        table = axs[3].table(cellText = [timeslot_used_glpk, timeslot_used_graph], cellLoc = 'center', rowLabels = rows, colLabels = columns, loc='top')
        table.auto_set_font_size(False)
        table.set_fontsize(8)
        table.scale(1, 1.3)

        time_used_glpk = [(time_used_glpk1[x] + time_used_glpk2[x] + time_used_glpk3[x]) / 3 for x in range(len(count))]
        time_used_graph = [(time_used_graph1[x] + time_used_graph2[x] + time_used_graph3[x]) / 3 for x in range(len(count))]
        ...
        axs[0].plot(smooth_count, smooth_time_used_glpk, marker='.', markevery = 10, label = 'GLPK')
        axs[0].plot(smooth_count, smooth_time_used_graph, marker='.', markevery = 10, label = 'Graph')
        axs[0].legend()
